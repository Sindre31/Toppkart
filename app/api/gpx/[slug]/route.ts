...
function buildGpx(tour: Tour): string {
...
<gpx version="1.1" creator="Toppkart" xmlns="http://www.topografix.com/GPX/1/1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.topografix.com/GPX/1/1 http://www.topografix.com/GPX/1/1/gpx.xsd">
  <metadata>
    ...
    <desc>${xml(`${tour.name} (${tour.region}) — skjematisk rutelinje fra Toppkart. Eksempeldata, ikke en kvalitetssikret rute.`)}</desc>
...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!tour) {
    return new NextResponse("Fant ikke turen.", { status: 404, headers: { "Content-Type": "text/plain; charset=utf-8" } });
  }
  ...
  return new NextResponse(buildGpx(tour), {
    status: 200,
    headers: {
