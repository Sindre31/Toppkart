...
lib/
  ...
  tours.ts               The 170 tours, REGIONS, getTour(), routesFor(), routeById(),
                         routeFor(), routeProfile()
  routes.ts              Generated ascent routes per tour — see scripts/build-routes/
  guides.ts              Editorial guide content — all 170 tours, generated
...
supabase/
  ...
  seed.sql               The 170 tours and all 170 guides
...
- **The tour cards are checked, the guides are checked, the geometry is checked — by machine.**
  `check_tours.py`, `check_guides.py` and `check_routes.py` come back clean on all 170 tours: every
...
- **The guide text has not been read by anyone who has skied these tours.** Every number in
  ...
  as bokmål ones, and comes back clean on all 170 guides.
  On top of that, **105 of the 170 have been through an independent adversarial read** whose only job
  ...
  `scripts/build-routes/README.md`. The sixty-five of the twelve newest rounds — Senja/Helgeland, Romsdal, Troms, Lofoten, Narvik, Sør-Troms, Tjeldsund, Lofast, Grytøya/Kvæfjord-vest, Ibestad, Tjeldøya and the island-completion round — are the current gap. It found four fabricated quotations, seven dropped or
