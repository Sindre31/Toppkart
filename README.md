...
lib/
  ...
  tours.ts               The 130 tours, REGIONS, getTour(), routesFor(), routeById(),
                         routeFor(), routeProfile()
  routes.ts              Generated ascent routes per tour — see scripts/build-routes/
  guides.ts              Editorial guide content — all 130 tours, generated
...
supabase/
  ...
  seed.sql               The 130 tours and all 130 guides
...
- **The tour cards are checked, the guides are checked, the geometry is checked — by machine.**
  `check_tours.py`, `check_guides.py` and `check_routes.py` come back clean on all 130 tours: every
...
- **The guide text has not been read by anyone who has skied these tours.** Every number in
  ...
  as bokmål ones, and comes back clean on all 130 guides.
  On top of that, **105 of the 130 have been through an independent adversarial read** whose only job
  ...
  `scripts/build-routes/README.md`. The twenty-five of the five newest rounds — Senja/Helgeland, Romsdal, Troms, Lofoten and Narvik — are the current gap. It found four fabricated quotations, seven dropped or
