...
lib/
  ...
  tours.ts               The 103 tours, REGIONS, getTour(), routesFor(), routeById(),
                         routeFor(), routeProfile()
  routes.ts              Generated ascent routes per tour — see scripts/build-routes/
  guides.ts              Editorial guide content — all 103 tours, generated
...
supabase/
  ...
  seed.sql               The 103 tours and all 103 guides
...
- **The tour cards are checked, the guides are checked, the geometry is checked — by machine.**
  `check_tours.py`, `check_guides.py` and `check_routes.py` come back clean on all 103 tours: every
...
- **The guide text has not been read by anyone who has skied these tours.** Every number in
  ...
  as bokmål ones, and comes back clean on all 103 guides.
  On top of that, **90 of the 103 have been through an independent adversarial read** whose only job
  ...
  The thirteen that have not — Gullfjellstoppen, the three Kvamskogen tours (Såta, Skrott,
  Tveitakvitingen), the two of the Voss round (Finnbufjellet, Vatnaknausen), the two of
  the continued popularity round (Varden (Småtindan), Midtitinden), the two of the
  backlog round (Fastdalstinden, Togga), the two of round four (Englafjell, Sandhornet)
  and Tomskjevelen — were written and checked by the same pass, which every earlier
  round's notes name as the gap an independent read exists to close.
