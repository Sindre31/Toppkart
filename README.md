...
lib/
  ...
  tours.ts               The 100 tours, REGIONS, getTour(), routesFor(), routeById(),
                         routeFor(), routeProfile()
  routes.ts              Generated ascent routes per tour — see scripts/build-routes/
  guides.ts              Editorial guide content — all 100 tours, generated
...
supabase/
  ...
  seed.sql               The 100 tours and all 100 guides
...
- **The tour cards are checked, the guides are checked, the geometry is checked — by machine.**
  `check_tours.py`, `check_guides.py` and `check_routes.py` come back clean on all 100 tours: every
...
- **The guide text has not been read by anyone who has skied these tours.** Every number in
  ...
  as bokmål ones, and comes back clean on all 100 guides.
  On top of that, **90 of the 100 have been through an independent adversarial read** whose only job
  ...
  The ten that have not — Gullfjellstoppen, the three Kvamskogen tours (Såta, Skrott,
  Tveitakvitingen), the two of the Voss round (Finnbufjellet, Vatnaknausen), the two of
  the continued popularity round (Varden (Småtindan), Midtitinden) and the two of the
  backlog round (Fastdalstinden, Togga) — were written and checked by the same pass,
